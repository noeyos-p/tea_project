...
document.addEventListener('DOMContentLoaded', () => {
  // 주요 DOM
  const scrollBox = document.getElementById('ptScroll'); // 가로 스크롤 컨테이너
  ...
  document.querySelectorAll('.pt-media').forEach(el => {
    ...
    if (isImg) {
      el.setAttribute('src', PLACEHOLDER_BG);           // 집계된 차들도 동일 이미지
      if (!el.getAttribute('alt')) el.setAttribute('alt', '차 이미지');
      el.style.backgroundImage = `url("${PLACEHOLDER_BG}")`;
    } else {
      el.style.backgroundImage = `url("${PLACEHOLDER_BG}")`;
    }
  ...
  // ===== 스크롤 네비게이션 =====
  function step() {
    return scrollBox ? Math.max(1, Math.floor(scrollBox.clientWidth * 0.9)) : 0;
  }
  function maxScroll() {
    if (!scrollBox) return 0;
    return Math.max(0, scrollBox.scrollWidth - scrollBox.clientWidth - 1); // 판정 보정
  }
  function updateNav() {
    ...
    prevBtn.classList.toggle('hidden', atStart);
    nextBtn.classList.toggle('hidden', atEnd || max === 0);
  }
  ...
  // ===== 로그인 링크 스무스 스크롤 =====
  const loginLink = document.getElementById('ptLoginLink');
